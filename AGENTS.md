...
- Local references are available at `../bobba_server`, `../bobba_client`, `../Kepler`, `../scuti-client`, `../scuti-renderer`, `../CycloneIO`, and `../Kepler-www`. Use them to understand emulator architecture, room-loop behavior, protocol shape, isometric rendering, static gamedata boundaries, and persistence boundaries.
- Treat these projects as behavioral references, not source templates. `bobba_server` and `bobba_client` are GPL-licensed, `Kepler` is AGPL-licensed, `CycloneIO` has GPLv3/manifest license ambiguity, and `scuti-client`, `scuti-renderer`, and `Kepler-www` have unclear or incomplete local licensing. Do not copy, port, or lightly rewrite their code, assets, SQL dumps, packet constants, text strings, schemas, filenames, bundle formats, proprietary Habbo-era content, or branded content into Tilezo.
...
- For rendering and UI work, use `../bobba_client` to study high-level client responsibilities such as asset managers, room imagers, avatar containers, hit testing, and incoming event routing. Use `../scuti-client` for client/renderer boundaries, UI state separation, message category organization, and asset-domain organization. Use `../scuti-renderer` for room visualization architecture, isometric projection, heightmap decomposition, layering, z sorting, hit testing, animation ticks, and asset loading lifecycle. Keep Tilezo's visual direction original and aligned with [docs/art-design-principles.md](docs/art-design-principles.md).
- Use `../CycloneIO` only as a secondary behavioral reference for broad monorepo boundaries, room join event flow, client event registry, in-memory room cache backed by persistence, and room-map domain concepts. Treat its client-driven movement as a negative reference; Tilezo movement must remain server-authoritative.
- Use `../Kepler-www` only to understand legacy static gamedata categories and hosting boundaries such as figure data, furnidata, external variables, external texts, DCR/CCT resources, `web-gallery`, and `c_images`. Do not use it as an implementation, asset, text, or file-format source.
- When a reference influences an implementation decision, mention the reference concept in the PR or commit notes without claiming compatibility with Habbo, Bobba, Kepler, Scuti, or Cyclone unless that compatibility is explicitly implemented and tested.
